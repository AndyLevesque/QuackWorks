...
cuboid([total_width, total_depth, plate_thickness], rounding = base_plate_radius, edges = [FRONT+LEFT, FRONT+RIGHT]){
    ...
    attach(TOP, BOT, align = BACK, overlap = 0.01)
        cuboid([total_width, plate_thickness, Top_Lip_Depth], rounding = Top_Lip_Depth / 2, edges = [TOP+LEFT, TOP+RIGHT])
            ...
            attach(BACK, BOT, inside = true, shiftout = 0.01)
                cuboid([pencil_hole_size,Top_Lip_Depth - 4, plate_thickness + 0.02], chamfer = - plate_thickness, edges=[BOT+LEFT, BOT+RIGHT]);
    ...
    tag("remove")
        attach(TOP) 
            down(text_depth/2-0.01) {
                back(text_size){
                    left(textMargin)
                        text3d(str(Screw_Separation_Distance > 60 ? "Spacing" : "Spcing"), size=text_size, font=font, h=text_depth, atype="ycenter", anchor=RIGHT);
                    right(textMargin)        
                        text3d(str(Screw_Separation_Distance, "MM"), size=text_size, font=font, h=text_depth, atype="ycenter");
                    
                }
                fwd(text_size){
                    left(textMargin)
                        text3d(str(Screw_Separation_Distance > 60 ? "Height" : "Hght"), size=text_size, font=font, h=text_depth, atype="ycenter", anchor=RIGHT);
                    right(textMargin)        
                        text3d(str(Screw_Distance_from_Drawer_Top, "MM"), size=text_size, font=font, h=text_depth, atype="ycenter");
